...
def verPagosVentas(request, pk):
    if request.method == "GET":
        ...
        return render(
            request,
            "base/ventas/verPagosHistoricoVenta.html",
...
class CrearVentaAsociado(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        obj_parametro_asociado = get_object_or_404(
            ParametroAsociado, asociado=obj_asociado
        )
        context.update(
            {
                ...
                "meses": MesTarifa.objects.filter(
                    id__lte=9000, id__gte=obj_parametro_asociado.primerMes_id
                ),
            }
        )
        return context
    ...
    def post(self, request, *args, **kwargs):
        with transaction.atomic():
            ...
            if forma_pago in ["CREDITO", "DESCUENTO NOMINA"]:
                objHistoricoVenta.primerMes = get_object_or_404(
                    MesTarifa, pk=request.POST["primerMes"]
                )
                objHistoricoVenta.cuotas = request.POST["cuotas"]
                objHistoricoVenta.valorCuotas = int(
                    request.POST["valorCuotas"].replace(".", "")
                )
                ...
                objHistoricoVenta.tasaInteres = get_object_or_404(
                    TasasInteresCredito, concepto=concepto
                )
                objHistoricoVenta.save()

            else:  # CONTADO u otras formas
                objHistoricoVenta.descuento = get_object_or_404(
                    PorcentajeDescuento, pk=1
                )
            ...
            if forma_pago == "CONTADO":
                crear_historico_pago(
                    valor_neto, mes_pk=9992, metodo_pago_pk=metodo_pago_pk
                )

            elif (
                forma_pago in ["CREDITO", "DESCUENTO NOMINA"]
                and concepto.strip() == "CREDITO PRODUCTO - 1.0 %"
            ):
                anticipo = int(request.POST["anticipo"].replace(".", ""))
                objHistoricoVenta.pendientePago -= anticipo
                objHistoricoVenta.save()
                crear_historico_pago(
                    anticipo, mes_pk=9988, metodo_pago_pk=metodo_pago_anticipo_pk
                )
            ...
            for key, value in request.POST.items():
                ...
                productos.append(
                    DetalleVenta(
                        ...
                        producto=producto,
                        precio=int(
                            request.POST.get(f"precio_{index}", "0").replace(".", "")
                        ),
                        cantidad=int(request.POST.get(f"cantidad_{index}", "0")),
                        totalBruto=int(
                            request.POST.get(f"totalBruto_{index}", "0").replace(
                                ".", ""
                            )
                        ),
                        totalNeto=int(
                            request.POST.get(f"totalConInteres_{index}", "0").replace(
                                ".", ""
                            )
                        ),
                    )
        ...
        return HttpResponseRedirect(
            reverse_lazy(
                "asociado:listarVentasAsociado",
                kwargs={"pkAsociado": self.kwargs["pkAsociado"]},
            )
        )
...
class EliminarDetalleVenta(UpdateView):
    ...
    def form_valid(self, form):
        ...
        if not self.validar_eliminacion():
            messages.error(
                self.request,
                "La venta no se ha podido eliminar ya que hay un pago asociado a la venta.",
            )
            return HttpResponseRedirect(
                reverse_lazy(
                    "asociado:listarVentasAsociado", args=[self.kwargs["pkAsociado"]]
                )
            )  # No guarda y regresa al formulario
    ...
    def validar_eliminacion(self):
        ...
        if venta.formaPago in ["CREDITO", "DESCUENTO NOMINA"]:
            ...
            if not HistorialPagos.objects.filter(
                asociado=self.kwargs["pkAsociado"], ventaHE=venta.pk
            ).exists():
                return True
